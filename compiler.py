...
TYPES = {"int", "bool", "str", "char"}#, list, tuple, set, dict, stream
...
class Compiler(object):
	...
	def isLiteral(self, s : str):
		return self.isInteger(s) or self.isBoolean(s) or s == "not True" or s == "not False" or self.isString(s) or self.isChar(s)
	...
	def whichValue(self, name : str):
		...
		if self.isLiteral(name):
			...
			if name in self.symbolTable:
				value = self.symbolTable[name].value
			else:
	...
	def assignStmt(self, type : str, mode : str = "VAR"):
		...
		y = self.nextToken()
		if (self.__token == "+" or self.__token == "-"):
			self.nextToken()
			...
			y += f" {self.__token}"
		if not (self.isBoolean(self.__token) or self.isInteger(self.__token) or self.isString(self.__token) or self.isChar(self.__token)):
			self.processError(f"SyntaxError: Expected boolean, integer, string, or char, got {self.__token}")
